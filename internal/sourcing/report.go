...
const (
	...
	glanceTitle       string = "## At a glance [(ℹ)]((https://github.com/jazcarate/sp/blob/master/docs/understanding_a_report.md#at-a-glance))"
	emptyHelp         string = "🗅 Starting out a new Split Chain and don't know \"what now?\". No problem! Check the [docs](https://github.com/jazcarate/sp/blob/master/docs/new_sp_now_what.md)"
...
` + emptyHelp + `{{end}}
` + operationsTitle + `
Current trust configuration: **{{ .Configuration }}** [(ℹ)](https://github.com/jazcarate/sp/blob/master/docs/understanding_a_report.md{{ .Configuration | ToMarkdownAnchor }})

### Log{{if .Log}} [(go to the last ⬇)](#op-{{ .LastOp }})
| # |  Operation | On | Note | Status |
| ---: | --- | --- | --- | ---: |{{ range .Log }}
|[{{ .ID }}](#op-{{ .ID }})<a id="op-{{ .ID }}"></a>|{{ .Operation | ToOpMarkdown }} <!-- Sign {{ .By}} {{ .Signature }} -->|{{ .On | ToTime }}|{{ .Note }}|{{if .Valid }}✅{{ else }}❓{{ end }}|{{ end }}{{ else }}
🌈 Fresh new 🌈{{ end }}
`
